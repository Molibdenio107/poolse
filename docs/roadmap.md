...
| # | Slice | Done when |
|---|---|---|
| 4.1 | Readings: record, list, chart per pool | ✅ Built in rounds 4 to 6 and never ticked here. An analysis is a *visit* — one moment, one author, one set of notes — with a value and its own unit per metric; nine metrics, a trend chart between analyses rather than over time, a printable report, a CSV export, and two ways in: the club's own spreadsheet or a laboratory's PDF read by the parser. **On the pool's own page**, as the note below asks |
...
| 5.3 | Tariffs and cost per period | ✅ **Both halves built.** *The bill as a record:* An electricity fatura filed on its meter by CPE — header, the registers on the dial, every billed line typed and priced, money in cents — typed by hand or read off a PDF into the same form by a flag-gated parser, previewed and committed through one route. Cost per period is a fact from the bill for a billed meter. *The tariff* (20 September 2026): every sub-meter behind the club's one ponto de entrega gets a €/unit rate, effective-dated with one live rate at a time, and its kWh become euros — an **estimate**, labelled as one, in its own section that never sums with a billed euro. A month any reading went unpriced is a dash rather than a partial bar, and the panel says "com base em 9 de 12 meses" |
| 5.4 | Period comparison, correlation with temperature | The insight the module exists for. **Part of it arrived early** (20 September 2026): POOLSE-28 gives cost per lesson hour, per bather and per m³ on a pool's page, and a turma's own share on its page — comparable across periods because the denominators are defined once. **The comparison half landed 20 September 2026**: every month on a meter's chart carries the same month a year earlier, in kWh and euros, comparing only the months that have both years — and when the implied €/kWh moved, the panel names both rates, so a higher bill on flat consumption reads as a tariff change rather than as a pool. **And the temperature half landed the same day**: monthly mean *outside air* temperature per site, cached from Open-Meteo's archive into `facility_climate_month`, shown as a column in the chart's record and as a sentence in the comparison panel — context, never a correction. Heating degree days are stored beside the mean, with the base they used. Flag-gated and **off by default**: the free tier is non-commercial, so `WEATHER_HISTORY_ENABLED` and the commercial-endpoint move are on the go-live checklist. This also closes POOLSE-28 AC 7 |
