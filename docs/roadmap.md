...
| 5.3 | Tariffs and cost per period | ⚠️ **First half built: the bill as a record.** An electricity fatura filed on its meter by CPE — header, the registers on the dial, every billed line typed and priced, money in cents — typed by hand or read off a PDF into the same form by a flag-gated parser, previewed and committed through one route. Cost per period is a fact from the bill for a billed meter; **still to build:** a tariff for sub-meters with no bill, so their kWh becomes euros too |
