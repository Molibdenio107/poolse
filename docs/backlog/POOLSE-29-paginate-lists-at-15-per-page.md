...
- **Decided 28 Aug.** One rule, written into `CONVENTIONS.md`: *a list is exempt only if its length is fixed by the data model or by a fixed window; anything whose length grows as the club takes on more people is paginated.* That answers all three — the year grids are twelve months, the hover card makes no request at all — and it answered the turmas week grid the same way, which the ticket had not anticipated: paging a week grid would empty Tuesday rather than shorten the page. An exemption covers the *control*, never the *fetch*: Encerramentos is exempt from paging and now takes `?year`, because a surface showing a fixed window must ask for one.
- Conflicts to resolve: POOLSE-15 AC 2 requires the hover card to show the **full** student list "with no truncation", which reads against "no list renders unbounded". POOLSE-08's "+X more after 8 names" is a display collapse, not pagination, and must not be reimplemented as a page. **Both resolved by the exemption rule:** the hover card is bounded by the turma's capacity and makes no request of its own, so AC 2 stands unchanged and the collapse stays a collapse.
