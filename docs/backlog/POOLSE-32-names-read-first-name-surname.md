...
- Sorting is by surname (the sort key), while display is first-name-first. **Decided 28 Aug: the first surname after the given names.** "Maria Joana Ferreira Silva Santos" files under Ferreira. Note this is deliberately *not* the surname the short name shows — abbreviation keeps the last surname because it identifies, filing uses the first because that is where somebody looks. One rule, in `name_sort_key()`.
...
- Boundary cases needing a decided answer: a single-part name ("Madonna") — display and abbreviation both return the one part, and it sorts as its own surname; a two-part name is already the abbreviated form; particles ("de", "da", "dos", "e") that belong to a compound surname must not be returned alone as the "last surname" — "Maria da Silva" must abbreviate to "Maria da Silva" or "Maria Silva", not "Maria da". **Decided 28 Aug: keep the particle** — "Maria da Silva". It is part of the surname to the person who has it, keeping it is never wrong, and dropping it buys three characters. `e` joins rather than attaches, so "de Sousa e Melo" is one surname unit and abbreviates whole.
...
- Schema: requires the Person name to exist as parts (given names, surnames), not a single `name` string. **Checked 28 Aug: they already are** — `first_name`/`last_name` on both `student` and `membership`. There is no string to split and no report to write, which removes what this ticket priced as its real cost. `last_name` holds every surname, so the unit-splitting happens at render time rather than in storage.
