...
| Somebody starting a trial every fortnight | `trial_claim`, keyed on the normalised signup address with a unique index, written inside the provisioning transaction — POOLSE-62. A refused signup leaves no tenant; a race ends with one. The refusal never says which signal fired. An operator can free an address in one click, and that is a row rather than a deletion. |
| The signup IP becoming a record | It is hashed with `SIGNUP_IP_SALT` and never stored, logged or returned raw, and `/admin` reports it only as a count of matching signups. **No salt configured means no hash at all** — an unsalted digest of an IPv4 address is reversible in minutes. |
| The platform role reaching a tenant's data | `poolse_platform` does **not** have `BYPASSRLS`. It is named in a `FOR SELECT` policy on eight tables — the eighth is two columns of `app_user`, added 14-09-2026 so a trial notice can record the address it was owed to — and holds no privilege on the rest, so `student`, `student_sensitive` and `invoice` are refused outright. `assertPlatformRoleIsNarrow()` refuses to boot if it is pointed at the owner. |
| The platform role changing more than a plan | Column-level `GRANT UPDATE` on seventeen named columns of `organization` — billing state, the two ceilings, the access states, and since POOLSE-63 `billing_mode` and `paid_through`. No INSERT and no DELETE: signup stays the only way a tenant comes into being, and the platform cannot rename or destroy a club. `archived_at` joined the list on 14-09-2026 so the trial clock could close its ladder, which is a soft delete the same login can undo. |
