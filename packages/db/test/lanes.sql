...
-- ---------------------------------------------------------------------------
-- Test 5 — a lane cannot be double-booked, and its neighbour is free
--
-- The exclusion constraint, recreated against `lane_id`. Recreating it is where a
-- typo silently produces a constraint matching nothing — and it fails open, so
-- nothing would notice until two groups met in the water.
-- ---------------------------------------------------------------------------

DO $$
DECLARE
  v_org uuid; v_pool uuid; v_season uuid; v_a uuid; v_b uuid;
  v_lane3 uuid; v_lane4 uuid; v_when timestamptz := '2026-10-06 18:00:00+00';
BEGIN
  v_org := '55555555-5555-5555-5555-555555555555';
  SELECT id INTO v_pool FROM pool WHERE organization_id = v_org AND name = 'Tanque Grande';
  SELECT id INTO v_lane3 FROM lane
   WHERE pool_id = v_pool AND position = 3 AND archived_at IS NULL;
  SELECT id INTO v_lane4 FROM lane
   WHERE pool_id = v_pool AND position = 4 AND archived_at IS NULL;
  ...
  VALUES (v_org, v_season, 'Infantis A', v_pool, v_lane3) RETURNING id INTO v_a;
  INSERT INTO class_group (organization_id, season_id, name, pool_id, lane_id)
  VALUES (v_org, v_season, 'Infantis B', v_pool, v_lane3) RETURNING id INTO v_b;

  INSERT INTO class_session (organization_id, class_group_id, pool_id, lane_id,
                             starts_at, duration_minutes)
  VALUES (v_org, v_a, v_pool, v_lane3, v_when, 45);

  -- Overlapping by real duration: 18:00–18:45 blocks 18:30.
  BEGIN
    INSERT INTO class_session (organization_id, class_group_id, pool_id, lane_id,
                               starts_at, duration_minutes)
    VALUES (v_org, v_b, v_pool, v_lane3, v_when + interval '30 minutes', 45);
    RAISE EXCEPTION 'FAIL test 5a: two turmas were put in the same lane at once';
  EXCEPTION WHEN exclusion_violation THEN NULL;
  END;

  -- The neighbouring lane is a different lane.
  INSERT INTO class_session (organization_id, class_group_id, pool_id, lane_id,
                             starts_at, duration_minutes)
  VALUES (v_org, v_b, v_pool, v_lane4, v_when, 45);

  -- Back-to-back in the same lane is not an overlap.
  INSERT INTO class_session (organization_id, class_group_id, pool_id, lane_id,
                             starts_at, duration_minutes)
  VALUES (v_org, v_b, v_pool, v_lane3, v_when + interval '45 minutes', 45);

  RAISE NOTICE 'PASS test 5: a lane cannot be double-booked; neighbours and back-to-back are free';
END $$;

-- ---------------------------------------------------------------------------
-- Test 6 — a cancelled session releases its lane
-- ---------------------------------------------------------------------------

DO $$
DECLARE
  v_org uuid; v_pool uuid; v_lane3 uuid; v_c uuid; v_season uuid;
  v_when timestamptz := '2026-10-06 18:00:00+00';
BEGIN
  v_org := '55555555-5555-5555-5555-555555555555';
  SELECT id INTO v_pool FROM pool WHERE organization_id = v_org AND name = 'Tanque Grande';
  SELECT id INTO v_lane3 FROM lane
  ...
  UPDATE class_session SET status = 'cancelled'
   WHERE lane_id = v_lane3 AND starts_at = v_when;

  INSERT INTO class_group (organization_id, season_id, name, pool_id, lane_id)
  VALUES (v_org, v_season, 'Infantis C', v_pool, v_lane3) RETURNING id INTO v_c;

  INSERT INTO class_session (organization_id, class_group_id, pool_id, lane_id,
                             starts_at, duration_minutes)
  VALUES (v_org, v_c, v_pool, v_lane3, v_when, 45);

  RAISE NOTICE 'PASS test 6: a cancelled session frees its lane';
END $$;
