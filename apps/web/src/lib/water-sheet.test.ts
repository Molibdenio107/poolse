...
test('a bare "Total" is a question, not a decision', () => {
  /*
   ...
   * What matters is that it arrives as `unsure`, the band the mapping step stops
   * and asks about, rather than as a confident match nobody is shown.
   *
   * The distinction is not cosmetic here. "Total" in a club's own sheet is as
   * likely to be a sum of something else, and a confident mapping would write a
   * plausible wrong number into a safety record.
   */
  ...
  assert.equal(total?.confidence, 'unsure', 'and proposed as a question');
  assert.equal(total?.reason, 'abbreviation');

  // The columns it is sure about are still placed without a question.
